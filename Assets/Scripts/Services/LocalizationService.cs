...
현재 맞추던 상태는 새 스크램블로 바뀝니다.	Your current progress will be replaced by a new scramble.	今揃えている状態は新しいスクランブルに置き換わります。	当前进度会被新的打乱替换。	目前進度會被新的打亂取代。	目前進度會被新嘅打亂取代。	Tu progreso actual se sustituirá por una nueva mezcla.	Votre progression actuelle sera remplacée par un nouveau mélange.	Dein aktueller Fortschritt wird durch eine neue Mischung ersetzt.	Seu progresso atual será substituído por um novo embaralhamento.	Текущий прогресс заменится новым перемешиванием.	Tiến trình hiện tại sẽ được thay bằng một lần trộn mới.	Progres saat ini akan diganti dengan acakan baru.	ความคืบหน้าปัจจุบันจะถูกแทนที่ด้วยการสับใหม่
...
카메라 권한이 꺼져 있습니다. 휴대폰 설정에서 직접 허용해 주세요.	Camera permission is off. Please allow it in your phone settings.	カメラの権限がオフです。端末の設定で許可してください。	相机权限已关闭。请在手机设置中开启。	相機權限已關閉。請在手機設定中開啟。	相機權限已關閉。請喺手機設定度開啟。	El permiso de cámara está desactivado. Actívalo en los ajustes del teléfono.	L'autorisation caméra est désactivée. Activez-la dans les réglages du téléphone.	Die Kameraberechtigung ist aus. Bitte in den Telefoneinstellungen erlauben.	A permissão da câmera está desativada. Ative nas configurações do telefone.	Доступ к камере выключен. Разрешите его в настройках телефона.	Quyền camera đang tắt. Hãy bật trong cài đặt điện thoại.	Izin kamera nonaktif. Aktifkan di pengaturan ponsel.	สิทธิ์กล้องถูกปิดอยู่ กรุณาอนุญาตในการตั้งค่าโทรศัพท์
카메라 촬영은 연결된 휴대폰에서 사용할 수 있습니다.	Camera scanning is available on a phone with a camera.	カメラ撮影はカメラのある端末で使えます。	相机拍摄需要在带相机的手机上使用。	相機拍攝需要在帶相機的手機上使用。	相機拍攝需要喺有相機嘅手機上用。	El escaneo con cámara está disponible en un teléfono con cámara.	Le scan par caméra est disponible sur un téléphone équipé d'une caméra.	Das Scannen mit der Kamera funktioniert auf einem Telefon mit Kamera.	A digitalização por câmera está disponível em um telefone com câmera.	Съёмка камерой доступна на телефоне с камерой.	Quét bằng camera chỉ dùng được trên điện thoại có camera.	Pemindaian kamera tersedia di ponsel yang punya kamera.	การสแกนด้วยกล้องใช้ได้บนโทรศัพท์ที่มีกล้อง
아래 면에 흰 십자를 만들 조각을 위층으로 올린 뒤 자리를 맞추고 내리세요.	Bring an edge for the white cross to the top layer, line it up, then drop it down.	下面の白い十字を作る辺を上段に上げ、位置を合わせてから下ろします。	把用于底面白十字的棱块转到顶层，对准位置后放下去。	把用於底面白十字的邊塊轉到頂層，對準位置後放下去。	將用嚟砌底面白十字嘅邊塊轉到頂層，對準位置後放落去。	Sube al nivel superior una arista para la cruz blanca, alinéala y bájala.	Montez en haut une arête de la croix blanche, alignez-la, puis descendez-la.	Bringe eine Kante fürs weiße Kreuz nach oben, richte sie aus und setze sie ab.	Leve para cima uma aresta da cruz branca, alinhe e desça.	Поднимите наверх ребро для белого креста, совместите и опустите.	Đưa cạnh dùng cho dấu cộng trắng lên tầng trên, canh đúng rồi hạ xuống.	Naikkan tepi untuk salib putih ke lapisan atas, sejajarkan, lalu turunkan.	ยกขอบที่ใช้ทำกากบาทขาวขึ้นชั้นบน จัดให้ตรงแล้วค่อยลง
흰색이 들어간 모서리를 위층으로 빼낸 뒤 들어갈 자리 위에 놓고 공식을 쓰세요.	Lift a corner containing white to the top, place it above its slot, then run the algorithm.	白を含む角を上段に出し、入る場所の真上に置いてから手順を使います。	把带白色的角块转到顶层，放到它该去的位置正上方，再用公式。	把帶白色的角塊轉到頂層，放到它該去的位置正上方，再用公式。	將帶白色嘅角塊轉到頂層，放到佢應該去嘅位置正上方，再用公式。	Saca al nivel superior una esquina con blanco, ponla sobre su hueco y aplica el algoritmo.	Faites remonter un coin contenant du blanc, placez-le au-dessus de son emplacement, puis appliquez l'algorithme.	Hole eine Ecke mit Weiß nach oben, stelle sie über ihren Platz und führe den Algorithmus aus.	Leve para cima um canto com branco, ponha sobre o lugar dele e aplique o algoritmo.	Выведите наверх угол с белым, поставьте над его местом и выполните алгоритм.	Đưa góc có màu trắng lên tầng trên, đặt ngay trên vị trí của nó rồi dùng công thức.	Angkat sudut yang memuat putih ke atas, letakkan di atas slotnya, lalu jalankan algoritma.	ยกมุมที่มีสีขาวขึ้นชั้นบน วางไว้เหนือช่องของมัน แล้วใช้สูตร
노란색이 없는 조각을 위층에서 찾아 앞면 색을 맞춘 뒤 공식을 쓰세요.	Find a top-layer edge without yellow, match its front color, then run the algorithm.	上段で黄色を含まない辺を探し、手前の色を合わせてから手順を使います。	在顶层找不含黄色的棱块，对准正面颜色后再用公式。	在頂層找不含黃色的邊塊，對準正面顏色後再用公式。	喺頂層搵唔含黃色嘅邊塊，對準正面顏色之後再用公式。	Busca arriba una arista sin amarillo, cuadra su color frontal y aplica el algoritmo.	Trouvez en haut une arête sans jaune, alignez sa couleur avant, puis appliquez l'algorithme.	Suche oben eine Kante ohne Gelb, richte ihre Vorderfarbe aus und führe den Algorithmus aus.	Ache em cima uma aresta sem amarelo, alinhe a cor da frente e aplique o algoritmo.	Найдите вверху ребро без жёлтого, совместите его передний цвет и выполните алгоритм.	Tìm cạnh tầng trên không có màu vàng, khớp màu mặt trước rồi dùng công thức.	Cari tepi lapisan atas tanpa kuning, cocokkan warna depannya, lalu jalankan algoritma.	หาขอบชั้นบนที่ไม่มีสีเหลือง จัดสีด้านหน้าให้ตรง แล้วใช้สูตร
다음 단계 설명을 다시 읽어 보세요.	Try reading the next stage's explanation again.	次のステップの説明をもう一度読んでみましょう。	再读一遍下一步的说明。	再讀一遍下一步的說明。	再睇多次下一步嘅說明。	Vuelve a leer la explicación de la siguiente etapa.	Relisez l'explication de l'étape suivante.	Lies die Erklärung der nächsten Stufe noch einmal.	Leia novamente a explicação da próxima etapa.	Перечитайте объяснение следующего этапа.	Hãy đọc lại phần giải thích của bước tiếp theo.	Coba baca lagi penjelasan tahap berikutnya.	ลองอ่านคำอธิบายของขั้นถัดไปอีกครั้ง
